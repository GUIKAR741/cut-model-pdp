...
class Graph():
    ...
    def initProblem(self, file):
        ...
        linha = arq.readline()
        p = int(linha.split(" ")[0])
        ed = int(linha.split(" ")[1].split('\n')[0])
        self.vertices = [i for i in range(1, p + 1)]
        for _ in range(p):
            linha = arq.readline()
            n1 = float(linha.split(',')[0])
            n2 = float(linha.split(',')[1].split('\n')[0])
            self.points.append((n1, n2))

        for _ in range(ed):
            linha = arq.readline()
            n1 = int(linha.split('-')[0])
            n2 = int(linha.split('-')[1].split(' ')[0])
            w = linha.split('-')[1].split(' ')[1].split('\n')[0]
            if(w == 'c'):
    ...
    def calculateDistances(self):
        ...
        for i in self.edge:
            s = i.split(',')[0]
            f = i.split(',')[1]
            self.distance[s].update({f: self.euclidianDistance(
                self.points[int(s) - 1],
                self.points[int(f) - 1]
            )})
    ...
    def plotCor(self, name):
        ...
        for i in self.edge:
            ...
            if not (i in self.edgeCuts):
                plt.plot(
                    [
                        self.points[int(f) - 1][0],
                        self.points[int(s) - 1][0]
                    ],
                    [
                        self.points[int(f) - 1][1],
                        self.points[int(s) - 1][1]
                    ],
                    'blue'
                )
    ...
    def plotDesloc(self, cuts, name):
        ...
        for i in range(len(cuts)):
            ...
            if(not (cuts[i] in save)) and \
                (not (str(str(f) + "," + str(s)) in save)) and \
                    (cuts[i] in self.edgeCuts or (str(str(f) + "," + str(s)) in self.edgeCuts)):
                plt.plot(
                    [
                        self.points[int(f) - 1][0],
                        self.points[int(s) - 1][0]
                ...
                plt.text(
                    (self.points[s - 1][0] + self.points[f - 1][0]) / 2,
                    (self.points[s - 1][1] + self.points[f - 1][1]) / 2,
                    str(i + 1)
                )
            else:
                plt.plot(
    ...
    def plotSolution3(self, cuts, name, fo):
            ...
            if (not (i in self.edgeCuts)) or (i in vis):
                if i in vis:
                    if self.points[int(s) - 1][0] == self.points[int(f) - 1][0]:
                        qv_d = plt.quiver(
                            self.points[int(s) - 1][0] + add_x,
                            self.points[int(s) - 1][1],
                            self.points[int(f) - 1][0] - self.points[int(s) - 1][0],
                            (self.points[int(f) - 1][1]) - self.points[int(s) - 1][1],
                            scale_units='xy',
                            angles='xy',
                            scale=1,
                            color='red'
                        ...
                        plt.text(
                            (self.points[s - 1][0] + self.points[f - 1][0]) / 2 + add_x,
                            (self.points[s - 1][1] + self.points[f - 1][1]) / 2,
                            str(count)
                        )
                        ...
                        plt.text(
                            (self.points[s - 1][0] + self.points[f - 1][0]) / 2,
                            (self.points[s - 1][1] + self.points[f - 1][1]) / 2 + add_y,
                            str(count)
                        )
                    else:
                        qv_d = plt.quiver(
        ...
        plt.quiverkey(
            qv_d,
            (plt.xlim()[1] // 2) - (plt.xlim()[1] // 2) * .5,
            plt.ylim()[1] + plt.ylim()[1] * .05,
            150,
            'Moves',
            coordinates='data'
        )
        ...
        plt.text(
            (plt.xlim()[1] // 2) - (plt.xlim()[1] // 2) * .35,
            plt.ylim()[1] + plt.ylim()[1] * .05,
            "Time Required: {:.2f}".format(fo)
        )
